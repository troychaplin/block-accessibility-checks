...
export function MetaIndicator({ issues, metaKey }) {
	...
	useEffect(() => {
		...
		const handleClickOutside = event => {
			...
			const popover = document.querySelector('.ba11y-meta-indicator-popover');
	...
	}, [isTooltipOpen, metaKey]);

	// Listen for close events from other indicators
	useEffect(() => {
		const handleCloseEvent = event => {
			if (event.detail.metaKey !== metaKey) {
				setIsTooltipOpen(false);
			}
		};
	...
	return (
		<div className={className} ref={buttonRef}>
			<Button
				...
				className="ba11y-meta-indicator-button"
			...
			{isTooltipOpen && (
				<Popover
					...
					className="ba11y-meta-indicator-popover"
					focusOnMount={false}
				>
