  ...
  if (block.name === 'core/button' && !block.attributes.url && !block.attributes.text.originalHTML) {
    ...
    switch (validationMode) {
      case 'error':
        ...
        response.message = (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Error: Buttons must have text and a link', 'block-accessibility-checks');
        break;
      case 'warning':
        response.isValid = false;
        response.message = (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Warning: Buttons must have text and a link', 'block-accessibility-checks');
        break;
      case 'none':
      default:
...
function checkHeadingLevel(block) {
  if (block.name === 'core/heading' && block.attributes.level === 1) {
    ...
    switch (validationMode) {
      case 'error':
        response.isValid = false;
        response.message = (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Error: Level 1 headings should only be used for page titles', 'block-accessibility-checks');
        break;
      case 'warning':
        response.isValid = false;
        response.message = (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Warning: Level 1 headings should only be used for page titles', 'block-accessibility-checks');
        break;
      case 'none':
      default:
        response.isValid = true;
...
function checkImageAlt(block) {
  if (block.name === 'core/image' && !block.attributes.alt && !block.attributes.isDecorative) {
    ...
    switch (validationMode) {
      case 'error':
        response.isValid = false;
        response.message = (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Error: Images are required to have alternative text', 'block-accessibility-checks');
        break;
      case 'warning':
        response.isValid = false;
        response.message = (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Warning: Images are required to have alternative text', 'block-accessibility-checks');
        break;
      case 'none':
      default:
        response.isValid = true;
...
function checkTableHeaderRow(block) {
  if (block.name === 'core/table' && block.attributes.body.length !== 0 && block.attributes.head.length === 0) {
    ...
    switch (validationMode) {
      case 'error':
        response.isValid = false;
        response.message = (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Error: Tables are required to have a header row', 'block-accessibility-checks');
        break;
      case 'warning':
        response.isValid = false;
        response.message = (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Warning: Tables are required to have a header row', 'block-accessibility-checks');
        break;
      case 'none':
      default:
        response.isValid = true;
...
function getInvalidBlocksRecursive(blocks) {
  ...
  return blocks.flatMap(function (block) {
    ...
    var results = _registerPlugin__WEBPACK_IMPORTED_MODULE_1__.blockChecksArray.map(function (check) {
      return check(block);
    });

    // If the block has inner blocks, recursively check them
    if (block.innerBlocks && block.innerBlocks.length > 0) {
      return [].concat(_toConsumableArray(results), _toConsumableArray(getInvalidBlocksRecursive(block.innerBlocks)));
    }
    return results;
  });
}
